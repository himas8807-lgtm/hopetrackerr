...
async function sharePoster() {
    ...
    try {
        ...
        if (isMobile) {
            // Mobile: Use pure Canvas API (no html2canvas)
            blob = await createCanvasPoster();
        } else {
        ...
        const file = new File([blob], 'bts-mission-results.png', { type: 'image/png' });
        ...
        if (!shared) {
            ...
            a.href = url;
            a.download = 'bts-mission-results.png';
            a.click();
            URL.revokeObjectURL(url);
            showToast('Saved! Share from gallery 💜', 'success');
        }

    } catch (e) {
        console.error('Share error:', e);
        showToast('Long-press the poster to save 📸', 'info');
    } finally {
        if (btn) {
            btn.disabled = false;
...
// ==================== CANVAS POSTER (Works on ALL devices) ====================
function createCanvasPoster() {
    return new Promise((resolve) => {
        ...
        headerGrad.addColorStop(0, '#4a1a7a');
        headerGrad.addColorStop(1, '#7b2cbf');
        ...
        ctx.fillText('INTELLIGENCE REPORT', W / 2, 55);

        ctx.fillStyle = '#ffffff';
        ctx.font = '900 26px Arial, sans-serif';
        ctx.fillText('BTS COMEBACK MISSION', W / 2, 95);

        // Week label from card
        const weekText = STATE.week || '';
        ctx.fillStyle = 'rgba(255,255,255,0.4)';
        ctx.font = '13px monospace';
        ctx.fillText(weekText, W / 2, 135);
        ...
        for (const div of allDivs) {
            ...
            if (style.includes('font-size:56px') || style.includes('font-size: 56px')) {
                totalStreams = div.textContent.trim();
                break;
            }
        }
        ...
        let trackTotal = '0';
        let albumTotal = '0';
        const headerDivs = card.querySelectorAll('div');
        for (const div of headerDivs) {
            const style = div.getAttribute('style') || '';
            const text = div.textContent.trim();
            if (style.includes('font-size:14px') && style.includes('font-weight:900') && style.includes('monospace')) {
                if (!trackTotal || trackTotal === '0') {
                    trackTotal = text;
                } else if (!albumTotal || albumTotal === '0') {
                    albumTotal = text;
                }
            }
        }
        ...
        // Track & Album items
        const columns = card.querySelectorAll('[style*="min-width:0"]') || 
                        card.querySelectorAll('[style*="min-width: 0"]');
        
        const trackItems = [];
        const albumItems = [];

        if (columns.length >= 2) {
            columns[0].querySelectorAll('[style*="margin-bottom:10px"], [style*="margin-bottom: 10px"]').forEach(item => {
                const divs = item.querySelectorAll('div');
                if (divs.length >= 2) {
                    trackItems.push({ name: divs[0].textContent.trim(), count: divs[1].textContent.trim() });
                }
            });
            columns[1].querySelectorAll('[style*="margin-bottom:10px"], [style*="margin-bottom: 10px"]').forEach(item => {
                const divs = item.querySelectorAll('div');
                if (divs.length >= 2) {
                    albumItems.push({ name: divs[0].textContent.trim(), count: divs[1].textContent.trim() });
                }
            });
        }
        ...
        ctx.textAlign = 'center';
        ctx.fillText(totalStreams, W / 2, 260);

        ctx.fillStyle = 'rgba(255,215,0,0.7)';
        ctx.font = '700 12px Arial, sans-serif';
        ctx.fillText('STREAMS RECORDED', W / 2, 290);
        ...
        ctx.fillText('🎵 TRACKS', leftX, y);

        ctx.fillStyle = '#fff';
        ctx.font = '900 16px monospace';
        ctx.fillText(trackTotal, leftX, y + 22);

        // Album header
        ctx.fillStyle = '#00d4ff';
        ctx.font = '800 12px Arial, sans-serif';
        ctx.fillText('💿 ALBUMS', rightX, y);

        ctx.fillStyle = '#fff';
        ctx.font = '900 16px monospace';
        ctx.fillText(albumTotal, rightX, y + 22);

        // === TRACK ITEMS ===
        y = 420;
        trackItems.slice(0, 5).forEach((item, i) => {
            const itemY = y + (i * 48);
            ...
            ctx.fillText(name, leftX, itemY);

            ctx.fillStyle = '#fff';
            ctx.font = 'bold 15px monospace';
            ctx.fillText(item.count, leftX, itemY + 18);
        });

        // === ALBUM ITEMS ===
        albumItems.slice(0, 5).forEach((item, i) => {
            const itemY = y + (i * 48);
            ...
            ctx.textAlign = 'left';
            const name = item.name.length > 22 ? item.name.substring(0, 22) + '…' : item.name;
            ctx.fillText(name, rightX, itemY);

            ctx.fillStyle = '#fff';
            ctx.font = 'bold 15px monospace';
            ctx.fillText(item.count, rightX, itemY + 18);
        });
        ...
        ctx.textAlign = 'center';
        ctx.fillText('HOPETRACKER', W / 2, H - 25);
...
window.createCanvasPoster = createCanvasPoster;
window.renderWeekConfirmation = renderWeekConfirmation;

...
document.getElementById('new-password')?.addEventListener('input', function() {
    ...
    strength.className = 'password-strength show';
    if (score <= 2) {
...
function checkPasswordMatch() {
    ...
    match.className = 'password-match show';
    if (newPass === confirmPass) {
        match.className += ' match';
        match.textContent = '✓ Passwords match';
    } else {
        match.className += ' no-match';
...
async function handlePasswordChange(event) {
    ...
    if (currentPassword === newPassword) {
        showToast('error', 'Error', 'New password must be different');
        return false;
    }
    ...
    if (!agentId) {
        showToast('error', 'Error', 'Session expired. Please login again.');
        return false;
    }
...
function initUpdateTimer() {
    ...
    setInterval(updateNextUpdateTime, 60000); // Update every minute
}
...
function updateNextUpdateTime() {
    ...
    const timeStr = minutes <= 0 ? 'Now' : `${minutes}m`;
    
    // Update sidebar
    const sidebarTime = document.getElementById('next-update-time');
    if (sidebarTime) sidebarTime.textContent = timeStr;
    
    // Update mobile header
    const mobileTime = document.getElementById('mobile-next-update');
...
function updateLastUpdatedTime(timestamp) {
    ...
    if (diff < 1) {
        ...
        timeStr = `${Math.floor(diff)}m ago`;
    } else {
        timeStr = `${Math.floor(diff / 60)}h ago`;
    }
    
    text.textContent = `Updated: ${timeStr}`;
    
    // Mark as stale if >1 hour old
    if (diff > 60) {
...
// Initialize on load
document.addEventListener('DOMContentLoaded', initUpdateTimer);
